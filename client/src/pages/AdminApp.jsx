...
export default function AdminApp() {
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      await axios.delete(`${API_URL}/api/admin/users/${userId}`, getAuth());
          ...
          <div className="bg-white p-6 rounded-2xl w-full max-w-lg shadow-2xl">
            ...
            <form onSubmit={handleUpdateUser} className="space-y-4">
              ...
              {editingUser.role === 'retailer' && (
                <input type="text" placeholder="Shop Name" value={editingUser.shopName || ''} onChange={e => setEditingUser({...editingUser, shopName: e.target.value})} className="w-full p-3 border rounded-lg outline-none" />
              )}
            ...
            <form onSubmit={handleCreateStaff} className="space-y-4">
              ...
              {(staffForm.role === 'retailer' || staffForm.role === 'delivery_agent') && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                  ...
                  <input type="text" placeholder="Detailed Address / Base Location" value={staffForm.address} onChange={(e) => setStaffForm({...staffForm, address: e.target.value})} required className="md:col-span-2 p-3 border rounded-lg outline-none" />
                    ...
                    <tr key={user._id} className="hover:bg-indigo-50 transition text-sm">
                      ...
                      <td className="p-3 border-b">
                        <div className="flex justify-center gap-2">
                          ...
                          <button onClick={() => handleDeleteUser(user._id)} className="bg-red-50 text-red-600 p-2 rounded hover:bg-red-100 transition" title="Delete User">
                  ...
                  <div key={product._id} className="flex flex-col md:flex-row items-center justify-between p-4 rounded-xl border bg-yellow-50 border-yellow-200 shadow-sm">
                    ...
                    <div className="flex flex-col gap-2 mt-4 md:mt-0 items-end bg-white p-3 rounded-lg border border-yellow-100 shadow-sm">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <button onClick={() => handleApproval(product._id, 'approve')} className="bg-green-600 text-white px-4 py-2.5 rounded-lg font-bold">Approve</button>
                        <button onClick={() => handleApproval(product._id, 'reject')} className="bg-red-50 text-red-600 px-4 py-2.5 rounded-lg font-bold">Reject</button>
                      </div>
                    </div>
                  </div>
                ))}
