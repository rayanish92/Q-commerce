...
export default function RetailerApp() {
  ...
  // NEW: Handle Order Acceptance / Rejection
  const handleOrderAction = async (orderId, subOrderId, action) => {
    try {
      ...
      setMessage(`Failed to process order action.`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="max-w-6xl mx-auto p-4 mt-4 w-full">
        ...
        {/* TAB 3: ORDERS LIST WITH ACCEPT/REJECT */}
        {activeTab === 'orders' && (
          <div className="bg-white p-6 rounded-xl shadow-sm">
            <h2 className="text-xl font-bold mb-6">Action Required: Orders</h2>
            
            <div className="space-y-4">
              {orders.length === 0 ? <p className="text-center p-4 text-gray-500 border rounded-lg font-bold">No orders assigned to you currently.</p> : null}
                ...
                <div key={order._id} className="border rounded-xl p-5 bg-gray-50 relative overflow-hidden">
                     ...
                     <ul className="space-y-1">
                       {order.subOrder.items.map((item, idx) => (
                         <li key={idx} className="font-bold text-gray-800 flex justify-between">
                            ...
                            <span className="text-gray-500">₹{item.price * item.cartQty}</span>
                         </li>
                       ))}
                     </ul>
                   </div>
                   ...
                   {order.subOrder.status === 'Pending' && (
                     <div className="flex gap-3 mt-4 pt-4 border-t">
                       <button onClick={() => handleOrderAction(order._id, order.subOrder._id, 'Accept')} className="flex-1 bg-green-600 text-white font-bold py-3 rounded-lg hover:bg-green-700 shadow-sm transition">
                         Accept Order
                       </button>
                       <button onClick={() => handleOrderAction(order._id, order.subOrder._id, 'Reject')} className="flex-1 bg-red-100 text-red-700 font-bold py-3 rounded-lg hover:bg-red-200 transition">
                         Reject
                       </button>
                     </div>
                   )}
