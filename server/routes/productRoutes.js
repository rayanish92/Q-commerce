...
// NEW: RETAILER REQUESTS A PRICE CHANGE
router.put('/request-price-change/:id', verifyRetailerOrAdmin, async (req, res) => {
  try {
    const updated = await Product.findByIdAndUpdate(req.params.id, { 
      retailerPrice: req.body.newPrice,
      status: 'Price Update Pending' 
    }, { new: true });
...
// Customer View uses sellingPrice
router.get('/nearby', verifyToken, async (req, res) => {
  try {
    ...
    const query = { retailerId: { $in: retailerIds }, status: 'Approved', quantity: { $gt: 0 } };
